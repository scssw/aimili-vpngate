...
在您的 **Debian 11+ / Ubuntu 20.04+** VPS 机器上，复制并运行以下一行指令即可完成自动安装部署：
...
##### 1. 极简系统缺少依赖（Debian 11+ / Ubuntu 20.04+ 首次安装）
...
##### 2. Debian 系统兼容运行方法
安装脚本已原生支持 Debian 11 或更高版本，无需再用 `sed` 修改系统类型判断。直接运行一键安装命令即可：
```bash
bash <(curl -Ls https://raw.githubusercontent.com/baoweise-bot/aimili-vpngate/main/install.sh)
```
...
To install and deploy AimiliVPN on your **Debian 11+ / Ubuntu 20.04+** server, copy and paste the following command:
...
##### 2. Debian Support
The installer now supports Debian 11 or newer directly. No `sed` patch is required:
```bash
bash <(curl -Ls https://raw.githubusercontent.com/baoweise-bot/aimili-vpngate/main/install.sh)
```
